...
# Функция для создания кнопок "Назад" и "Критерии"
def get_buttons():
    keyboard = [
...
async def nickname(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("Отлично! Теперь, пожалуйста, укажи свой игровой айди.", reply_markup=get_buttons())
    return PLAYER_ID
...
async def kd_current(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("Какой у тебя КД за прошлый сезон?", reply_markup=get_buttons())
    return KD_PREVIOUS
...
async def matches_previous(update: Update, context: CallbackContext) -> int:
    ...
    await update.message.reply_text("Пожалуйста, отправь скриншот статистики из игры за текущий сезон.", reply_markup=get_buttons())
    return SCREENSHOT_1
...
async def screenshot_1(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.photo:
        ...
        await update.message.reply_text("Теперь отправь скриншот статистики из игры за прошлый сезон.", reply_markup=get_buttons())
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if query.data == 'back':  # Кнопка "Назад"
        return await go_back(update, context)
    elif query.data == 'reset':  # Проверяем callback_data
...
async def go_back(update: Update, context: CallbackContext) -> int:
    ...
    previous_step = context.user_data.get('previous_step', READY)
